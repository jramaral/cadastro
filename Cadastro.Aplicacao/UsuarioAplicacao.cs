    ...
    public class UsuarioAplicacao
    {
        ...
        public void NewUsuario(Usuario usuario)
        {
            var query = string.Format(
                "INSERT Into Usuario(Nome, Cpf, sexo, telefone, DataCadastro, Cidade, Email, Senha) Values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}')",
                usuario.Nome, usuario.Cpf, usuario.Sexo, usuario.Telefone,
                string.Format("{0:yyyy-MM-dd}", usuario.DataCadastro), usuario.Cidade, usuario.Email, usuario.Senha);
